...
from lidar_catenary.explore_data import print_summary, save_3d_plot
from lidar_catenary.read_data import read_lidar_parquet
...
def cluster_command(file_path, eps, min_samples):
    ...
    from lidar_catenary.cluster_data import (
        cluster_point_cloud,
...
def cluster_all_command(eps, min_samples):
    ...
    from lidar_catenary.cluster_data import cluster_point_cloud, get_cluster_summary, save_cluster_plot
...
def fit_command(file_path, eps, min_samples):
    ...
    from lidar_catenary.fit_catenary import (
    ...
    point_cloud = read_lidar_parquet(file_path)
    clustered_data, local_points, center, directions, fit_results = fit_catenaries_for_point_cloud(
        point_cloud,
        eps=eps,
    ...
    output_3d_plot = Path("fit_images") / f"{input_file.stem}_3d_catenary_fits.png"
    ...
    save_3d_fit_plot(clustered_data, local_points, center, directions, fit_results, output_3d_plot)
    save_fit_summary_csv(fit_results, output_csv)

    return fit_results


def fit_all_command(eps, min_samples):
    """Fit catenary curves for every LiDAR Parquet file and save one combined CSV."""
    from lidar_catenary.fit_catenary import (
        fit_catenaries_for_point_cloud,
        save_3d_fit_plot,
        save_fit_plot,
    ...
    for file_path in parquet_files:
        ...
        output_plot = Path("fit_images") / f"{file_path.stem}_catenary_fits.png"
        output_3d_plot = Path("fit_images") / f"{file_path.stem}_3d_catenary_fits.png"
        output_csv = Path("fit_images") / f"{file_path.stem}_fit_summary.csv"
        save_fit_plot(clustered_data, local_points, fit_results, output_plot)
        save_3d_fit_plot(
            clustered_data,
            local_points,
        ...
        for result in fit_results:
            all_rows.append({"file": file_path.name, **result})

    output_csv = Path("fit_images") / "fit_summary.csv"
    output_csv.parent.mkdir(parents=True, exist_ok=True)
    with output_csv.open("w", newline="") as csv_file:
        fieldnames = ["file", "cluster", "point_count", "rmse", "s0", "z0", "c"]
        writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(all_rows)

    print("Combined catenary fit summary:")
    print(f"{'file':45} {'cluster':>7} {'points':>8} {'rmse':>10}")
    for row in all_rows:
        print(
            f"{row['file']:45} "
            f"{row['cluster']:>7} "
    ...
    print(f"Saved combined fit summary: {output_csv}")
...
def build_parser():
    ...
    add_clustering_arguments(cluster_all_parser)
    ...
    add_clustering_arguments(fit_parser)

    fit_all_parser = subparsers.add_parser(
        "fit-all",
        help="Fit catenary curves for all lidar_cable_points_*.parquet files.",
    )
    add_clustering_arguments(fit_all_parser)

    return parser


def add_clustering_arguments(parser):
    """Add shared DBSCAN arguments to a subcommand parser."""
    parser.add_argument(
        "--eps",
        type=float,
        default=0.35,
...
def main():
    ...
    if args.command == "read":
        ...
        fit_all_command(args.eps, args.min_samples)
